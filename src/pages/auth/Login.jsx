...
export default function Login() {
  const { signInWithGoogle, signInMock, signInWithEmail } = useAuth()
  const [searchParams] = useSearchParams()
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState(null)

  // Real Email & Password state
        ...
        <div style={{
          ...
          {isMock ? (
            ...
            <form onSubmit={handleEmailLogin} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div className="input-group">
                <label className="input-label" style={{ fontWeight: 600, display: 'block', marginBottom: 4, fontSize: 12 }}>Alamat Email</label>
                <input
                  type="email"
                  value={email}
                  ...
                  style={{ 
                    fontSize: 13, height: 38, width: '100%', padding: '6px 10px', 
                    borderRadius: 6, border: '1px solid var(--gray-200)', boxSizing: 'border-box',
                    outline: 'none', background: 'var(--surface)', color: 'var(--gray-800)'
                  }}
                  required
                />
              </div>
              ...
              <div className="input-group">
                <label className="input-label" style={{ fontWeight: 600, display: 'block', marginBottom: 4, fontSize: 12 }}>Kata Sandi (Password)</label>
                <input
                  type="password"
                  value={password}
              ...
              >
                {loading ? <MiniSpinner /> : null}
                <span>{loading ? 'Masuk...' : 'Masuk dengan Email'}</span>
              </button>

              {/* Divider */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '8px 0' }}>
                <div style={{ flex: 1, borderTop: '1px solid var(--gray-200)' }} />
              ...
              <button
                ...
                style={{
                  width: '100%',
                  display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10,
                  padding: '10px 16px', borderRadius: 6, border: '1px solid var(--gray-300)',
                  backgroundColor: '#ffffff',
                  color: 'var(--gray-700)', fontSize: 14, fontWeight: 600,
                  cursor: loading ? 'not-allowed' : 'pointer',
                ...
                onMouseEnter={e => { if (!loading) { e.currentTarget.style.backgroundColor = 'var(--gray-50)' } }}
